...
class OptimizationQCA(BaseQCA):
    ...
    def dataset_store(
        ...
        Args:
            dataset (OptimizationDataset): Dataset to store.
        ...
        def _assemble_data(rec: OptimizationRecord) -> tuple:
            rec_dict = rec.dict()

            props = rec_dict["properties"]
            energy = props["results"]["scf_total_energy"] * h_2_kcalmol
            geometry = (
                props["results"]["geometry"] * bohr_2_angstroms
